...
class RoomSegmentMetadataStore(private val dao:SessionDao):SegmentMetadataStore{
 ...
 override suspend fun saveReady(segment:ReadySegment){val old=dao.segment(segment.id.value);if(old!=null&&old.state!=SegmentState.OPEN.name)return;dao.saveSegment(AudioSegmentEntity(segment.id.value,segment.blockId.value,old?.ordinal?:0,segment.path,File(segment.path).length(),segment.durationMs,segment.sha256,SegmentState.READY.name,old?.transcriptionAttempts?:0))}
