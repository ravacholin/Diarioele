...
@Dao interface SessionDao {
 ...
 @Insert(onConflict=OnConflictStrategy.REPLACE) suspend fun saveEditorialReport(report:EditorialReportEntity)
 @Query("SELECT * FROM editorial_reports WHERE sessionId=:sessionId LIMIT 1") suspend fun editorialReport(sessionId:String):EditorialReportEntity?
 @Query("SELECT * FROM editorial_reports WHERE sessionId=:sessionId LIMIT 1") fun observeEditorialReport(sessionId:String):Flow<EditorialReportEntity?>
 @Query("SELECT editorial_reports.* FROM editorial_reports INNER JOIN sessions ON sessions.id=editorial_reports.sessionId ORDER BY sessions.updatedAtEpochMs DESC LIMIT 1") fun observeLatestEditorialReport():Flow<EditorialReportEntity?>
 @Query("SELECT * FROM editorial_reports WHERE sessionId=:sessionId AND inputHash=:inputHash AND state='READY' LIMIT 1") suspend fun readyEditorialReport(sessionId:String,inputHash:String):EditorialReportEntity?
 @Query("UPDATE editorial_reports SET state='READY',rawJson=:rawJson,summary=:summary,materialText=:materialText,homeworkText=:homeworkText,provider=:provider,modelId=:modelId,promptVersion=:promptVersion,schemaVersion=:schemaVersion,validatorVersion=:validatorVersion,failure=NULL,updatedAtEpochMs=:updatedAtEpochMs WHERE sessionId=:sessionId AND inputHash=:inputHash AND state='GENERATING'") suspend fun saveEditorialReadyIfCurrent(sessionId:String,inputHash:String,rawJson:String,summary:String,materialText:String,homeworkText:String,provider:String?,modelId:String?,promptVersion:String,schemaVersion:String,validatorVersion:String,updatedAtEpochMs:Long):Int
 @Query("UPDATE editorial_reports SET state='FAILED',failure=:failure,updatedAtEpochMs=:updatedAtEpochMs WHERE sessionId=:sessionId AND inputHash=:inputHash AND state='GENERATING'") suspend fun markEditorialFailedIfCurrent(sessionId:String,inputHash:String,failure:String,updatedAtEpochMs:Long):Int
 @Query("UPDATE editorial_reports SET state='STALE',failure=NULL,updatedAtEpochMs=:updatedAtEpochMs WHERE sessionId=:sessionId") suspend fun markEditorialReportStale(sessionId:String,updatedAtEpochMs:Long):Int
 @Query("DELETE FROM editorial_reports WHERE sessionId=:sessionId") suspend fun deleteEditorialReport(sessionId:String)
}
