...
class CaptureViewModel(repository:SessionRepository,private val actions:CaptureActions,scope:CoroutineScope?=null,drafts:Flow<DiaryDraftEntity?> = flowOf(null),claims:Flow<List<EvidenceClaim>> = flowOf(emptyList()),pendingRecordings:Flow<RecordingReport?> = repository.observeLatestCleanupPendingRecording(),diaries:Flow<List<DiaryEntry>> = flowOf(emptyList()),semanticRuns:Flow<SemanticRunUi?> = flowOf(null),observeProgress:(SessionId)->Flow<TranscriptionProgress?> = { flowOf(null) },recordingFailures:Flow<RecordingFailure?> = flowOf(null)):ViewModel(){
 ...
 private fun reportState(report:RecordingReport,draft:DiaryDraftEntity?,evidence:List<EvidenceClaim>,progress:TranscriptionProgress?,semantic:SemanticRunUi?=null):CaptureUiState{
 ...
 private fun recordingFailureMessage(failure:RecordingFailure)=when(failure){RecordingFailure.OPUS_UNAVAILABLE->"Este teléfono no pudo iniciar OGG/Opus. La grabación no comenzó";RecordingFailure.STORAGE->"La grabación se interrumpió por un error de almacenamiento. El audio cerrado sigue guardado";RecordingFailure.CAPTURE->"La grabación se interrumpió. El audio cerrado sigue guardado y podés reanudar"}
