...
class CaptureViewModel(repository:SessionRepository,private val actions:CaptureActions,scope:CoroutineScope?=null,drafts:Flow<DiaryDraftEntity?> = flowOf(null),claims:Flow<List<EvidenceClaim>> = flowOf(emptyList()),pendingRecordings:Flow<RecordingReport?> = repository.observeLatestCleanupPendingRecording(),diaries:Flow<List<DiaryEntry>> = flowOf(emptyList()),observeProgress:(SessionId)->Flow<TranscriptionProgress?> = { flowOf(null) }):ViewModel(){
 ...
 init {workScope.launch{combine(repository.observeActiveSession().map{it as Any?},repository.observeLatestFinalizedRecording().map{it as Any?},drafts.map{it as Any?},claims.map{it as Any?},pendingRecordings.map{it as Any?},diaries.map{it as Any?},progressFlow.map{it as Any?}){values->values}.collect{values->
  val session=values[0] as SessionAggregate?;val report=values[1] as RecordingReport?;val draft=values[2] as DiaryDraftEntity?;@Suppress("UNCHECKED_CAST") val evidence=values[3] as List<EvidenceClaim>;val pending=values[4] as RecordingReport?;@Suppress("UNCHECKED_CAST") val entries=values[5] as List<DiaryEntry>;val progress=values[6] as TranscriptionProgress?
 ...
 private fun reportState(report:RecordingReport,draft:DiaryDraftEntity?,evidence:List<EvidenceClaim>,progress:TranscriptionProgress?):CaptureUiState{
