...
import com.capo.diarioclase.processing.editorial.EditorialReportCodec
...
class CaptureViewModel(repository:SessionRepository,private val actions:CaptureActions,scope:CoroutineScope?=null,drafts:Flow<DiaryDraftEntity?> = flowOf(null),claims:Flow<List<EvidenceClaim>> = flowOf(emptyList()),pendingRecordings:Flow<RecordingReport?> = repository.observeLatestCleanupPendingRecording(),diaries:Flow<List<DiaryEntry>> = flowOf(emptyList()),semanticRuns:Flow<SemanticRunUi?> = flowOf(null),interpretations:Flow<InterpretationProgressUi?> = flowOf(null),editorialReports:Flow<EditorialReportEntity?> = flowOf(null),observeProgress:(SessionId)->Flow<TranscriptionProgress?> = { flowOf(null) }):ViewModel(){
 ...
 private fun reportState(report:RecordingReport,draft:DiaryDraftEntity?,evidence:List<EvidenceClaim>,progress:TranscriptionProgress?,semantic:SemanticRunUi?=null,interpretation:InterpretationProgressUi?=null,editorial:EditorialReportUi?=null):CaptureUiState{
 ...
 private fun EditorialReportEntity.toUi():EditorialReportUi{val parsed=if(state==EditorialReportState.READY.name&&rawJson.isNotBlank())runCatching{EditorialReportCodec.decode(rawJson)}.getOrNull() else null;return EditorialReportUi(runCatching{EditorialReportState.valueOf(state)}.getOrDefault(EditorialReportState.FAILED),parsed,provider,failure)}
 ...
 fun onRegenerateEditorialReport(mode:InterpretationMode){val draft=_state.value.draft?:return;execute(clearOnSuccess=true){actions.regenerateEditorialReport(SessionId(draft.sessionId),mode)}}
 ...
 // Corpus local opt-in (Fase 6, Q7): guardar el ejemplo anonimizado de esta sesión o borrar todo.
 fun onSaveExample(){val id=reviewSessionId()?:return;execute(clearOnSuccess=true){actions.saveExample(id);_state.update{it.copy(message="Ejemplo guardado en el corpus local")}}}
 fun onDeleteAllExamples(){execute(clearOnSuccess=true){actions.deleteAllExamples();_state.update{it.copy(message="Corpus local borrado")}}}
 fun onExportCorpus(uri:String){execute(clearOnSuccess=true){actions.exportCorpus(uri);_state.update{it.copy(message="Corpus exportado")}}}
 fun onImportCorpus(uri:String){execute(clearOnSuccess=true){actions.importCorpus(uri);_state.update{it.copy(message="Corpus importado")}}}
 ...
 fun onApprove(topics:String,activities:String,pages:String,exercises:String,homework:String){val current=_state.value;val draft=current.draft?:return;if(current.busy||current.status!=CaptureStatus.DRAFT||current.editorial?.state!=EditorialReportState.READY||current.editorial.report==null)return;val edited=draft.copy(topics=topics,activities=activities,pages=pages,exercises=exercises,homework=homework,userEdited=true);_state.value=current.copy(status=CaptureStatus.APPROVING,busy=true,message="Guardando la ficha permanente",draft=edited);workScope.launch{try{showApprovalOutcome(actions.approveAndClean(edited))}catch(error:CancellationException){throw error}catch(error:Exception){_state.update{it.copy(status=CaptureStatus.DRAFT,busy=false,message=error.message?:"No se pudo aprobar la ficha")}}}}
