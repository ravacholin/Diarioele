...
fun CaptureScreen(state:CaptureUiState,onStart:()->Unit,onPause:()->Unit,onResume:()->Unit,onMark:()->Unit,onFinalize:()->Unit,onProcess:(InterpretationMode)->Unit,onMode:(InterpretationMode)->Unit,onPauseProcessing:()->Unit,onResumeProcessing:(InterpretationMode)->Unit,onContinueLocal:(InterpretationMode)->Unit,onSave:(String,String,String,String,String)->Unit,onApprove:(String,String,String,String,String)->Unit,onRetryCleanup:()->Unit,interpretationMode:InterpretationMode=InterpretationMode.CONSERVATIVE,onAcceptClaim:(String)->Unit={},onRejectClaim:(String)->Unit={},onCorrectClaim:(String,String)->Unit={_,_->},onSaveExample:()->Unit={},onDeleteExamples:()->Unit={},onExportCorpus:()->Unit={},onImportCorpus:()->Unit={},onRetryInterpretation:()->Unit={}){
 MaterialTheme(colorScheme=darkColorScheme(primary=Color.White,onPrimary=Color.Black,secondary=Color.LightGray,onSecondary=Color.Black,onSurfaceVariant=Color.LightGray,surfaceVariant=Color.DarkGray,error=Color.White,onError=Color.Black,background=Color.Black,surface=Color.Black,onBackground=Color.White,onSurface=Color.White,outline=Color.Gray)){
  when(state.status){CaptureStatus.REVIEW,CaptureStatus.PROCESSING->state.lastRecording?.let{RecordingReportScreen(state,it,onStart,onProcess,onPauseProcessing,onResumeProcessing,onContinueLocal,interpretationMode)};CaptureStatus.DRAFT->DraftScreen(state,onMode,onSave,onApprove,onAcceptClaim,onRejectClaim,onCorrectClaim,onSaveExample,onDeleteExamples,onExportCorpus,onImportCorpus,onRetryInterpretation);CaptureStatus.APPROVING->ApprovalProgressScreen(state);CaptureStatus.CLEANUP_PENDING,CaptureStatus.ARCHIVED->PermanentDiaryScreen(state,onRetryCleanup,onStart);else->CaptureControls(state,onStart,onPause,onResume,onMark,onFinalize)}
 }
}
...
@Composable private fun DraftScreen(state:CaptureUiState,onMode:(InterpretationMode)->Unit,onSave:(String,String,String,String,String)->Unit,onApprove:(String,String,String,String,String)->Unit,onAcceptClaim:(String)->Unit,onRejectClaim:(String)->Unit,onCorrectClaim:(String,String)->Unit,onSaveExample:()->Unit,onDeleteExamples:()->Unit,onExportCorpus:()->Unit,onImportCorpus:()->Unit,onRetryInterpretation:()->Unit){
 ...
 LazyColumn(Modifier.fillMaxSize().background(Color.Black).padding(horizontal=24.dp),verticalArrangement=Arrangement.spacedBy(18.dp)){
  ...
  if(draft.summary.isNotBlank())item{Text("RESUMEN",fontSize=12.sp,fontWeight=FontWeight.Bold,letterSpacing=2.sp);Spacer(Modifier.height(8.dp));Text(draft.summary,color=Color.LightGray,fontSize=14.sp)}
