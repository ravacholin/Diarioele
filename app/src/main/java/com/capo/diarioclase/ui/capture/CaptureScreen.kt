...
@Composable private fun ReviewableSection(title:String,claims:List<EvidenceClaim>,enabled:Boolean,onAccept:(String)->Unit,onReject:(String)->Unit,onCorrect:(EvidenceClaim)->Unit){
 ...
 claims.forEach{claim->Column(Modifier.fillMaxWidth().border(1.dp,Color.DarkGray).padding(12.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){
  Text("${categoryLabel(claim.category)}  ${"%.0f".format(claim.confidence*100)}%",fontSize=11.sp,color=Color.LightGray);Text(claim.value,fontWeight=FontWeight.Bold);Text("“${claim.evidence.excerpt}”",color=Color.LightGray,fontSize=12.sp)
