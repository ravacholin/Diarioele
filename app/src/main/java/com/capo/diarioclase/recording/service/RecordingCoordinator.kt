...
class RecordingCoordinator(private val sessions:SessionRepository,private val segments:SegmentStore,private val sourceFactory:()->PcmSource,private val clock:Clock,private val scope:CoroutineScope,private val onUnexpectedStop:(Throwable)->Unit={}) {
 ...
 suspend fun recordBlock(block:BlockId,source:PcmSource){
  ...
  try {
   while(currentCoroutineContext().isActive){val read=source.read(buffer);if(read<0)break;if(read==0)continue;var offset=0
    while(offset<read){if(current==null)current=segments.open(block,ordinal);val take=minOf(read-offset,MAX_SEGMENT_SAMPLES-currentSamples);segments.append(current,buffer.copyOfRange(offset,offset+take),take);offset+=take;currentSamples+=take
     if(currentSamples==MAX_SEGMENT_SAMPLES){segments.close(current);current=null;currentSamples=0;ordinal++}
    }
   }
