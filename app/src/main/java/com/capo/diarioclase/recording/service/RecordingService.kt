...
class RecordingService:Service(){
 ...
 override fun onCreate(){super.onCreate();RecordingNotification.createChannel(this);startForeground(RecordingNotification.ID,RecordingNotification.build(this));val app=application as DiarioClaseApp;coordinator=RecordingCoordinator(sessions=app.repository,segments=app.recordingSegments,sourceFactory={app.recordingComponents.requireOpusSupport();AndroidPcmSource(this)},clock=SystemClock,scope=scope,maxSegmentSamples=16_000*60,onUnexpectedStop={stopForeground(STOP_FOREGROUND_REMOVE);stopSelf()})}
 override fun onStartCommand(intent:Intent?,flags:Int,startId:Int):Int{when(intent?.action){ACTION_START->intent.getStringExtra(EXTRA_SESSION_ID)?.let{id->scope.launch{runCatching{coordinator.start(SessionId(id))}.onFailure{stopForeground(STOP_FOREGROUND_REMOVE);stopSelf()}}};ACTION_PAUSE->scope.launch{coordinator.pause();stopForeground(STOP_FOREGROUND_REMOVE);stopSelf()};ACTION_FINALIZE->scope.launch{coordinator.finalizeDay();stopForeground(STOP_FOREGROUND_REMOVE);stopSelf()}};return START_NOT_STICKY}
